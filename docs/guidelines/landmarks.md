...
* facilitate sighted keyboard-only users to navigate to different sections of a page, using a browser add-on
* programmatically identify sections of a page
* convey the basic semantic meaning, of the various areas of the page
* clearly identify the relationships and meaning, of elements on the page
...
* Element with `role="region"` (usually `section`) must have a label.  Use the `aria-label` or `aria-labelledby` attribute to label it.
...
**Note**: New HTML5 tags have an implicit role, e.g tag `nav` has the `navigation` role, tag `footer` has `contentinfo` role.  Using the `role` attribute is, therefore, redundant but, because Joomla supports Internet Explorer 11, which does not support new HTML5 tags, we also use the role attribute.   
